//Fact general
import 'package:flutter/material.dart';

const int kFetchLimit = 10;
const double kDefaultPadding = 10;


//Fact constants
const String kAppName = 'DailyFactsNG';
const String kAppVersion = '1.0.0';
const String kContactEmail = 'nonybrighto@gmail.com';
const int kFactsPerDay = 10;


// prefKeys

const String kFactsPerDayPrefKey = 'facts_per_day';


//colors
const Color kPrimaryColor = Color(0XFF2DBB54);
const Color kSecondaryColor = kPrimaryColor;
const Color kTextColor = Color(0XFF4F4F4F);
const Color kGradientFirstColor = kPrimaryColor;
const Color kGradientSecondColor = Color(0XFF196a29);
const Color kGradientThirdColor = kPrimaryColor;
// const Color kGradientSecondColor = Color(0XFF1f8434);